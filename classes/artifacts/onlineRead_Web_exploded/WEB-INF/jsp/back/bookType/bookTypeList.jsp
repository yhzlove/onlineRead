...
<div class="content-wrapper">
    <div class="row">
        <div class="col-lg-12 heading">
            <h1 class="page-header">图书类别管理</h1>
            <div class="option-buttons">
                <div class="btn-toolbar" role="toolbar">
                    <div class="btn-group dropdown">
                        <a class="btn dropdown-toggle" href="${ctx}/back/bookTypeUpdate"><i class="en-plus s32"></i></a>
                    </div>
                </div>
            </div>
        </div>
                    ...
                    <div class="panel-body">
                        <table class="table display" id="datatable">
                            ...
                            <c:forEach items="${bookTypeList}" var="bookType" >
                                <tr class="odd gradeX">
                                    <td><img src="${ctx}${bookType.headImg}" alt="${user.userName}" style="width: 30px;"/></td>
                                    <td>${bookType.name}</td>
                                    <td>${bookType.bookCounts}</td>
                                    <td>${bookType.orderNo}</td>
                                    <td><fmt:formatDate value="${bookType.registTime}" pattern="yyyy/MM/dd" /></td>
                                    <td>
                                        <a href="${ctx}/back/bookTypeUpdate?id=${bookType.id}"><i class="ec-pencil s24"></i>修改</a>
                                        <a href="${ctx}/back/bookTypeDelete?id=${bookType.id}" onclick="return confirm('是否删除该数据')"><i class="fa-remove s24"></i>删除</a>
                                    </td>
                                </tr>
                            </c:forEach>
                            </tbody>
